...
(function () {
	...
	function(getDataOnMap,getGeocode,$timeout,$state,getMarkerData){
				...
				if (place.address_components) {
					...
					if (streetNumber && street){
						...
						getMarkerData.get({streetNum:streetNumber,streetName:street,city:town,state:state,zipCode:postCode,apartmentNumber:''}).then(function(data){
							...
							ctrl.listingData.numReviewsText = !isNaN(ctrl.listingData.numReviews) && ctrl.listingData.numReviews > 0 ? 'See '+ctrl.listingData.numReviews +' Comments': 'Add Comment';
							var contentString = '<p>'+ctrl.listingData.streetNum+' ' +ctrl.listingData.street+', '+ctrl.listingData.city+', '+ctrl.listingData.state+'</p>'+
							'<img src="https://maps.googleapis.com/maps/api/streetview?size=160x80&location=' + address2 + '&key=AIzaSyBqqOMuVejvk6bD4FatH4-N0y1iN7hQXmk">';
							if (!isNaN(ctrl.listingData.rating) && ctrl.listingData.rating> 0)
								contentString = contentString +'<br>' + ctrl.listingData.rating+' Star Rating '
							for (var i = 0 ; i < ctrl.listingData.rating; i++)
								contentString = contentString + '<span class="glyphicon glyphicon-star"></span>';
							contentString = contentString + '<br>' 
							            +'<a href="/#/reviews/'+(ctrl.listingData.listing?ctrl.listingData.listing.id:'')+'/'+ctrl.listingData.streetNum+'/'+ctrl.listingData.street+'/'+ctrl.listingData.city+'/'+ctrl.listingData.state+'/'+ctrl.listingData.zipCode+'/'+ctrl.listingData.country+'" class="btn btn-xs btn-success" style="margin-top:3px" role="button">'+ctrl.listingData.numReviewsText+'</a><br>'
							iw.setContent(contentString);
		...
		function closeIws(){
			ctrl.iws.forEach(function(item){
		...
		function LoadMap(lat,lng){
			...
			geocoder.geocode({'location': latlng}, function(results, status) {
				if (Array.isArray(results) && results.length > 0){
					var postCode = extractFromAddress(results[0].address_components, "postal_code");
					var street = extractFromAddress(results[0].address_components, "route");
					var town = extractFromAddress(results[0].address_components, "locality");
					var country = extractFromAddress(results[0].address_components, "country");
					var streetNumber = extractFromAddress(results[0].address_components, "street_number");
					var state = extractFromAddress(results[0].address_components, "administrative_area_level_1");
					if (postCode){ 
						setOtherMarkers(map,{
							street:street,
							streetNum:streetNumber,
		...
		function setOtherMarkers(map,home){				
			
			getDataOnMap.get(home).then(function(data){
				var iw = new google.maps.InfoWindow({
					maxWidth:160
				});
				ctrl.iws.push(iw);
				...
				for (var i = 0; i < data.length ; i++){
					data[i].image = 'https://maps.googleapis.com/maps/api/streetview?size=160x80&location=' + data[i].houseNumber+' ' +data[i].street+', '+data[i].city+', '+data[i].region+', United States&key=AIzaSyBqqOMuVejvk6bD4FatH4-N0y1iN7hQXmk';
					data[i].imageLg = 'https://maps.googleapis.com/maps/api/streetview?size=320x160&location=' + data[i].houseNumber+' ' +data[i].street+', '+data[i].city+', '+data[i].region+', United States&key=AIzaSyBqqOMuVejvk6bD4FatH4-N0y1iN7hQXmk';
				}
				...
				geocoder.geocode( { 'address': item.houseNumber+' '+item.street+', '+item.city+', '+item.region }, function(results, status) {
				  if (status == google.maps.GeocoderStatus.OK) {
					var contentString = '<p>'+item.houseNumber+' ' +item.street+', '+item.city+', '+item.region+'</p>'+
					'<img src="'+item.image+'">';
					if (!isNaN(item.ratingNum) && item.ratingNum> 0)
						contentString = contentString +'<br>' + item.ratingNum+' Star Rating '
					for (var i = 0 ; i < item.ratingNum; i++)
						contentString = contentString + '<span class="glyphicon glyphicon-star"></span>';
					contentString = contentString + '<br>' 
								+'<a href="/#/reviews/'+(item.id?item.id:'')+'/'+item.houseNumber+'/'+item.street+'/'+item.city+'/'+item.region+'/'+item.postalCode+'/United States'+'" class="btn btn-xs btn-success" style="margin-top:3px" role="button">'+ctrl.listingData.numReviewsText+'</a><br>'
					iw.setContent(contentString);
